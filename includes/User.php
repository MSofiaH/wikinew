...
class User implements IDBAccessObject {
	...
	protected function loadFromRow( $row, $data = null ) {
        ...
        if ( isset( $row->user_cuenta ) ) {
            $this->mCuenta = $row->user_cuenta;
            $this->setItemLoaded( 'cuenta' );
        } else {
            $all = false;
        }

        if ( isset( $row->user_carrera ) ) {
            $this->mCarrera = $row->user_carrera;
            $this->setItemLoaded( 'carrera' );
        } else {
            $all = false;
        }

        if ( isset( $row->user_campus ) ) {
            $this->mCampus = $row->user_campus;
            $this->setItemLoaded( 'campus' );
        } else {
            $all = false;
        }

        if ( isset( $row->user_catedratico ) ) {
            $this->mCatedratico = $row->user_catedratico;
            $this->setItemLoaded( 'catedratico' );
        } else {
            $all = false;
        }
	...
	public static function createNew( $name, $params = array() ) {
		...
		$fields = array(
            ...
            'user_cuenta' => $user->mCuenta,
            'user_carrera' => $user->mCarrera,
            'user_campus' => $user->mCampus,
            'user_catedratico' => $user->mCatedratico,
		);
	...
	public function addToDatabase() {
		...
		$dbw->insert( 'user',
			array(
                ...
                'user_cuenta' => $this->mCuenta,
                'user_carrera' => $this->mCarrera,
                'user_campus' => $this-> mCampus,
                'user_catedratico' => $this->mCatedratico,
			), __METHOD__,
			array( 'IGNORE' )
		);
		if ( !$dbw->affectedRows() ) {
    ...
    /**
     * Get Cuenta, Carrera, Catedratico, Campus
     */
    public function getCuenta() {
        if ( !$this->isItemLoaded( 'cuenta' ) ) {
            $this->load();
        }

        return $this->mCuenta;
    }
    public function setCuenta( $str ) {
        $this->load();
        $this->mCuenta = $str;
    }
    public function getCarrera() {
        if ( !$this->isItemLoaded( 'carrera' ) ) {
            $this->load();
        }

        return $this->mCarrera;
    }
    public function setCarrera( $str ) {
        $this->load();
        $this->mCarrera = $str;
    }
    public function getCampus() {
        if ( !$this->isItemLoaded( 'campus' ) ) {
            $this->load();
        }

        return $this->mCampus;
    }
    public function setCampus( $str ) {
        $this->load();
        $this->mCampus = $str;
    }

    public function getCatedratico() {
        if ( !$this->isItemLoaded( 'catedratico' ) ) {
            $this->load();
        }

        return $this->mCatedratico;
    }
    public function setCatedratico( $str ) {
        $this->load();
        $this->mCatedratico = $str;
    }
}
